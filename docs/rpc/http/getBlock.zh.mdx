...
- `<object>` - 如果块被确认，则具有以下字段的对象：
  ...
  - `previousBlockhash: <string>` - 该块的父块的块哈希，作为 base-58 编码字符串；如果由于账本清理导致父区块不可用，该字段将返回“11111111111111111111111111111111”
  - `parentSlot: <u64>` - 该块的父块的槽索引
  - `transactions: <array>` - 如果需要“full(完整)”交易详细信息，请提供；JSON 对象数组，包含：
    - `transaction: <object|[string,encoding]>` -
      [交易](/docs/rpc/json-structures.zh#交易)对象，可以是 JSON 格式，也可以是编码的二进制数据，具体取决于编码参数
    - `meta: <object>` - 交易状态元数据对象，包含`null` 或：
      ...
      - `innerInstructions: <array|null>` - [内部指令](/docs/rpc/json-structures.zh#内部指令) 列表 或 `null`在此交易期间未启用内部指令记录
      - `preTokenBalances: <array|undefined>` - [处理交易之前的代币余额](/docs/rpc/json-structures.zh#代币余额) 列表 ，如果在此交易期间尚未启用代币余额记录，则忽略该列表
      - `postTokenBalances: <array|undefined>` -[交易处理后的代币余额](/docs/rpc/json-structures.zh#代币余额) 列表， 如果在此交易期间尚未启用代币余额记录，则忽略该列表
      - `logMessages: <array|null>` - 字符串日志消息数组，或者`null`在此事务期间未启用日志消息记录
      - `rewards: <array|null>` - 交易级奖励，如果请求奖励则填充；JSON 对象数组，包含：
        ...
        - `commission: <u8|undefined>` - 奖励入账时的投票账户佣金，仅用于投票和质押奖励
      - 已弃用: `status: <object>` - 交易状态
        - `"Ok": <null>` - 交易成功
        - `"Err": <ERR>` - 交易失败并出现 TransactionError
      - `loadedAddresses: <object|undefined>` - 从地址查找表加载的交易地址。
        `maxSupportedTransactionVersion` 如果请求参数中未设置，或者`jsonParsed`请求参数中设置了编码，则未定义 。
  ...
  - `signatures: <array>` - 如果交易详细信息需要“签名”，请出示；签名字符串数组，对应于区块中的交易顺序
  - `rewards: <array|undefined>` - 区块级奖励，如果需要奖励则显示；JSON 对象数组，包含：
    - `pubkey: <string>` - 接收奖励的账户的公钥，采用 base-58 编码字符串
    - `lamports: <i64>`- 帐户贷记或借记的奖励灯数量，作为 i64
    - `postBalance: <u64>` - 应用奖励后lamports的账户余额
    - `rewardType: <string|undefined>` - 奖励类型： "fee(费用)", "rent(租金)",
      "voting(投票)", "staking(质押)"
    - `commission: <u8|undefined>` -  奖励入账时的投票账户佣金，仅用于投票和质押奖励
  - `blockTime: <i64|null>` - 估计的生产时间，作为 Unix 时间戳（自 Unix 纪元以来的秒数）。如果不可用则为 null
