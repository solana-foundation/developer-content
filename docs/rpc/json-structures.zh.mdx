...
交易与其他区块链上的交易完全不同。 一定要查
看[交易剖析](/docs/core/transactions.md)来学习Solana的交易情况
...
- `message: <object>` - 定义交易的内容。
  ...
  - `instructions: <array[object]>` - 如果全部成功，将按顺序执行并在一个原子事务
    中提交的程序指令列表。
    - `programIdIndex: <number>` - 数组索引，`message.accountKeys`指示执行该指令
      的程序帐户。
    - `accounts: <array[number]>` - 数组中的有序索引列表，
      `message.accountKeys`指示要传递给程序的帐户。
