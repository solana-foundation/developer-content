...
- Use **data validation checks** to verify that account data matches an expected
  value**.** Without appropriate data validations checks, unexpected accounts
  may be used in an instruction.
- To implement data validations checks in Rust, simply compare the data stored
  on an account to an expected value.
  ```rust
...
For this demo we’ll create a simple “vault” program similar to the program we
used in the Signer Authorization lesson and the Owner Check lesson. Similar to
those demos, we’ll show in this demo how a missing data validation check could
